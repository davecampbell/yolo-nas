#!/usr/bin/env python3
"""
Train YOLO-NAS model on custom dataset.

This script trains a YOLO-NAS model using SuperGradients on a custom dataset
exported from CVAT in YOLO format.

Usage:
    python train_custom_yolo_nas.py --config configs/training_config.yaml --dataset_config configs/dataset_config.yaml
"""
...
def create_dataloaders(dataset_config: dict, training_config: dict):
    ...
    train_dataloader = coco_detection_yolo_format_train(
        dataset_params={
            ...
            'labels_dir': dataset_config['train_labels_dir'],
    ...
    val_dataloader = coco_detection_yolo_format_val(
        dataset_params={
            ...
            'labels_dir': dataset_config['val_labels_dir'],
            'classes': dataset_config['classes'],
            'input_dim': dataset_config['input_dim'],
            'cache_images': dataset_config.get('cache_images', False),
            'cache_format': dataset_config.get('cache_format', 'ram'),
...
def main():
    ...
    parser.add_argument('--dataset_config', required=True, help='Path to dataset configuration file')
    ...
    train_dataloader, val_dataloader = create_dataloaders(dataset_config, training_config)
